...
import { Upload, AlertCircle, Star, Plus, FileText, Clock, Info } from 'lucide-react';
...
const FileUploader: React.FC<FileUploaderProps> = ({ 
  ...
  const processExcelFile = async (buffer: ArrayBuffer): Promise<Pub[]> => {
    try {
      ...
      const columnMappings = {
        ...
        install_date: headerRow.findIndex(h => /^(install.*date|installation.*date)$/i.test(h)),
        last_visited: headerRow.findIndex(h => /^(last.*visit|previous.*visit)$/i.test(h)),
        rtm: headerRow.findIndex(h => /^(rtm|route.*manager)$/i.test(h)),
        landlord: headerRow.findIndex(h => /^(landlord|owner|licensee)$/i.test(h)),
        notes: headerRow.findIndex(h => /^(notes|comments|remarks)$/i.test(h))
      };
      ...
      for (let i = 1; i < jsonData.length; i++) {
        ...
        if (!pubName || !postcode) {
          skippedRows.push({
            row: rowNum,
            reason: `Missing ${!pubName ? 'pub name' : ''}${!pubName && !postcode ? ' and ' : ''}${!postcode ? 'postcode' : ''}`
          });
          continue;
        }
        ...
        validPubs.push({
          pub: pubName,
          zip: postcode,
          install_date: columnMappings.install_date >= 0 ? row[columnMappings.install_date] || null : null,
          last_visited: columnMappings.last_visited >= 0 ? row[columnMappings.last_visited] || null : null,
          rtm: columnMappings.rtm >= 0 ? String(row[columnMappings.rtm] || '').trim() : '',
          landlord: columnMappings.landlord >= 0 ? String(row[columnMappings.landlord] || '').trim() : '',
          notes: columnMappings.notes >= 0 ? String(row[columnMappings.notes] || '').trim() : ''
        });
      }
      ...
      if (skippedRows.length > 0) {
        ...
        setError(`Warning: ${skippedRows.length} rows were skipped. Check console for details.`);
      }
  ...
  const validatePostcodeFormat = (postcode: string): boolean => {
    try {
      ...
      console.debug('Validating postcode format:', { original: postcode, cleaned: cleanPostcode });
  ...
  const validatePostcodes = async (pubs: Pub[]): Promise<void> => {
    try {
      ...
      if (invalidPostcodes.length > 0) {
        ...
        throw new Error(`Invalid postcode format found in ${invalidPostcodes.length} entries`);
      }
      ...
      if (!geocoder) {
        ...
        throw new Error('Geocoding service not available - please check API key and enabled services');
      }
  ...
  }, [fileType, setUserFiles, isFileLoaded]);
  ...
  const handleTypeDialogSubmit = (type: string, deadline?: string, priorityLevel?: number, followUpDays?: number) => {
    ...
    setUserFiles(prev => ({
      ...
      pubs: [...prev.pubs, ...processedPubs.map(pub => ({
        ...pub,
        fileName: currentFileName,
        fileId,
        uploadTime,

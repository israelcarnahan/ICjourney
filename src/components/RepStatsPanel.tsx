...
import { TrendingUp, Target, MapPin, Clock, AlertTriangle, CheckCircle2, Info, AlertCircle } from 'lucide-react';
import { differenceInDays, format, addBusinessDays } from 'date-fns';
...
const RepStatsPanel: React.FC = () => {
  ...
  const calculateStats = () => {
    const getListStats = (fileId: string, pubs: Pub[], type: string) => {
      ...
      const scheduledCount = schedule.reduce((acc, day) => 
        acc + day.visits.filter(v => v.fileId === fileId).length, 0
      );

      // Calculate completion date and days to deadline
      const completionDate = lastScheduledDate ? new Date(lastScheduledDate) : null;
      const deadline = firstPub?.deadline ? new Date(firstPub.deadline) : null;
      const daysToDeadline = deadline && completionDate ? 
        differenceInDays(deadline, completionDate) : null;
    ...
    [
      { pubs: allPubs.filter(pub => pub.listType === 'wins'), type: 'RepslyWin' },
      { pubs: allPubs.filter(pub => pub.listType === 'hitlist'), type: 'Wishlist' },
      { pubs: allPubs.filter(pub => pub.listType === 'unvisited'), type: 'Unvisited' },
      { pubs: allPubs.filter(pub => pub.listType === 'masterhouse'), type: 'Masterfile' }
    ].forEach(({ pubs, type }) => {
  ...
  const getStatusColor = (stat: any) => {
    ...
    if (stat.scheduled === 0) return 'bg-red-900/20 border-red-700/50 text-red-200';
  ...
  const getStatusIcon = (stat: any) => {
    ...
    if (stat.scheduled === 0) return <AlertCircle className="h-4 w-4 text-red-400" />;
  ...
  const getStatusMessage = (stat: any) => {
    ...
    let scheduledText = stat.isExhausted ? `All ${stat.total} scheduled` : 
                       stat.scheduled === 0 ? 'None scheduled' :
  ...
  const getPriorityInfo = (stat: any) => {
    ...
    if (stat.priority === 'RepslyWin') {
      return stat.followUpDays ? `Follow-up: ${stat.followUpDays} days after install` : 'Recent Win';
  ...
  return (
    <div className="animated-border bg-gradient-to-r from-eggplant-900/90 via-dark-900/95 to-eggplant-900/90 backdrop-blur-sm rounded-lg p-4">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h3 className="text-lg font-semibold text-eggplant-100">Schedule Overview</h3>
      </div>

      <div className="space-y-4">
        <div className="grid grid-cols-2 gap-4 mb-4">
          <div className="bg-eggplant-800/30 rounded-lg p-3">
            <div className="text-sm text-eggplant-200 mb-1">Total Scheduled</div>
            ...
            <div className="text-xs text-eggplant-300 mt-1">
              {schedule.length} days planned
            </div>
          </div>
          
          <div className="bg-eggplant-800/30 rounded-lg p-3">
            <div className="text-sm text-eggplant-200 mb-1">Average Per Day</div>
            <div className="text-xl font-bold text-eggplant-100">
              {schedule.length > 0 
                ? (schedule.reduce((acc, day) => acc + day.visits.length, 0) / schedule.length).toFixed(1)
                : '0'} visits
            </div>
            <div className="text-xs text-eggplant-300 mt-1">
              Target efficiency
        ...
        {Array.from(stats.entries()).map(([fileId, stat]) => (
          stat?.total > 0 && (
            <Tooltip.Provider key={`tooltip-${fileId}`}>
              <Tooltip.Root>
                <Tooltip.Trigger asChild>
                  <div
                    className={`flex flex-col gap-2 p-2.5 rounded-lg cursor-help ${getStatusColor(stat)}`}
                  >
                    <div className="flex items-center gap-2">
                      <div className="flex items-center gap-2">
                        {getStatusIcon(stat)}
                        <span className="font-medium text-sm whitespace-nowrap">{stat.fileName}</span>
                      </div>
                    </div>
                    <div className="flex items-center justify-between mt-1">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs">{getStatusMessage(stat)}</span>
                      </div>
                    </div>
                  </div>
                </Tooltip.Trigger>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="text-right">
            <p className="text-sm font-medium text-eggplant-100">
              {schedule.reduce((acc, day) => acc + (day.totalMileage || 0), 0).toFixed(1)} miles
            </p>
            <p className="text-xs text-eggplant-300">
              {(schedule.reduce((acc, day) => acc + (day.totalMileage || 0), 0) / Math.max(1, schedule.length)).toFixed(1)} miles per day
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="text-right">
            <p className="text-sm font-medium text-eggplant-100">
              {schedule.reduce((acc, day) => acc + (day.totalDriveTime || 0), 0)} minutes
            </p>
            <p className="text-xs text-eggplant-300">
              {(schedule.reduce((acc, day) => acc + (day.totalDriveTime || 0), 0) / Math.max(1, schedule.length)).toFixed(0)} minutes per day
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between mt-4 pt-4 border-t border-eggplant-800/30">
          ...
          <div className="text-right">
            <p className="text-sm font-medium text-eggplant-100">
              {Math.round((schedule.reduce((acc, day) => acc + day.visits.length, 0) / userFiles.pubs.length) * 100)}% of territory
            </p>
            <p className="text-xs text-eggplant-300">
              {schedule.reduce((acc, day) => acc + day.visits.length, 0)} of {userFiles.pubs.length} pubs
            </p>
          </div>
        </div>
      </div>
